...
function SolChat() {
  ...
  const showCalendly = name => {
    ...
    setTimeout(() => {
      ...
      setMsgs(m => [...m, {
        who: "sol",
        text: name ? `Great, ${name}. Pick a time that works for you and we'll see you on the call.` : "Pick a time that works for you and we'll see you on the call.",
        calendly: true
      }]);
      setStage("done");
    }, 800);
  ...
  const begin = () => {
    setStarted(true);
    sol("Hey! I'm Sol, the AI behind Sunsuite. I can book your strategy call or answer a quick question. What can I do for you?", () => setChips(["Book a strategy call", "What is Sunsuite?", "Talk to a human"]));
  };
  React.useEffect(() => {
    if (open && !started) setTimeout(begin, 350);
  }, [open]);
  const pick = label => {
    setMsgs(m => [...m, {
      who: "me",
    ...
    if (label === "Book a strategy call") {
      ...
      sol("Sunsuite is our own 34-tool platform: rankings, paid media, AI-search visibility, social, reviews and reporting in one place. I run it and send a morning briefing to every client. Most agencies resell tools — we built ours.", () => setChips(["Book a strategy call", "Talk to a human"]));
    } else if (label === "Talk to a human") {
      setStage("done");
      sol("Of course. Jon and Trevor are reachable at (303) 218-8570 or admin@sundigitalmarketing.com. You can also book a call below.", () => setChips(["Book a strategy call"]));
  ...
  const send = () => {
    ...
    if (stage === "booking_name") {
      ...
      sol(`Nice to meet you, ${name}. A strategy call is 30 minutes — we run your business through Sunsuite live and show you exactly what we'd work on first. Ready to pick a time?`, () => setChips(["Yes, book it", "No thanks"]));
      ...
      if (lower.includes("book") || lower.includes("call") || lower.includes("schedule") || lower.includes("meet")) {
        ...
        sol("Pricing depends on the mix of services and account size. The strategy call is the right place to get a real number — we run your business through Sunsuite and quote from there. Want to book one?", () => setChips(["Book a strategy call", "No thanks"]));
      } else if (lower.includes("seo") || lower.includes("google") || lower.includes("ranking") || lower.includes("search")) {
        sol("SEO is one of our core services, including local SEO, Google Business Profile, and AI-search visibility on ChatGPT, Gemini and Perplexity. Want to see what that looks like for your business?", () => setChips(["Book a strategy call", "What is Sunsuite?"]));
      } else if (lower.includes("sunsuite") || lower.includes("platform") || lower.includes("tool") || lower.includes("ai")) {
        pick("What is Sunsuite?");
        return;
      } else {
        sol("Good question. The best way to get a real answer for your specific business is a 30-minute strategy call — it's free and we run your numbers live. Want to book one?", () => setChips(["Book a strategy call", "Talk to a human"]));
      }
    }
  };
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, m.text, m.calendly && /*#__PURE__*/React.createElement("a", {
    href: CALENDLY_URL,
    target: "_blank",
  ...
  }, /*#__PURE__*/React.createElement("span", null), /*#__PURE__*/React.createElement("span", null), /*#__PURE__*/React.createElement("span", null)), chips.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "sc-chips"
